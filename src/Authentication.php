...
{
    public const LOGIN_ERROR = 'GEBRUIKERSACCOUNT_ONBEKEND';

    public function login_regicare_show(): bool|string
    {
        global $wp;
    ...
    public function forgot_password_show(): bool|string
    {
        global $wp;
        ...
        if (isset($_REQUEST['error']) && 'false' === $_REQUEST['error']) {
            ?>
            <p class="sb-text-center sb-login-warning">
                Als dit emailadres bij ons bekend is heeft u een email met instructies ontvangen.
            </p>
            <?php
        } else {
            ?>
            <form method="post" enctype="multipart/form-data" id="forgotPasswordForm">
                <div class="form-group">
                    <label for="regicare_email">Email address</label>
                    <input type="email" name="regicare_email" class="form-control" id="regicare_email" aria-describedby="email" placeholder="email adres" required>
                </div>
    ...
    public function register_regicare_show(): bool|string
    {
        global $wp;
        ...
        if (isset($_SESSION['error']['code'])) {
            ...
            echo match ($error['code']) {
                'GEBRUIKERSACCOUNT_INCOMPLEET', 'FORMDATAMISSING' => '<p class="sb-text-center sb-login-warning">De opgegeven data voor het opslaan van het gebruikersaccount is incompleet.</p>',
                'GEBRUIKERSACCOUNT_BEZET' => '<p class="sb-text-center sb-login-warning">De opgegeven e-mailadres bestaat al.</p>',
                'ADRES_ONBEKEND'          => '<p class="sb-text-center sb-login-warning">Het opgegeven adress voor het opslaan van het gebruikersaccount is incorrect.</p>',
                default                   => '<p class="sb-text-center sb-login-warning">Er ging iets fout met het aanmaken van het gebruikersaccount probeer het later nog eens.</p>',
            };
        }
    ...
    public function register_child_regicare_show(): bool|string
    {
        global $wp;
        ...
        if (isset($_SESSION['error']['code'])) {
            ...
            echo match ($error['code']) {
                'FORMDATAMISSING', 'GEBRUIKERSACCOUNT_INCOMPLEET' => '<p class="sb-text-center sb-login-warning">De opgegeven data voor het opslaan van het gebruikersaccount is incompleet.</p>',
                'GEBRUIKERSACCOUNT_BEZET' => '<p class="sb-text-center sb-login-warning">De opgegeven e-mailadres bestaat al.</p>',
                'ADRES_ONBEKEND'          => '<p class="sb-text-center sb-login-warning">Het opgegeven adress voor het opslaan van het gebruikersaccount is incorrect.</p>',
                default                   => '<p class="sb-text-center sb-login-warning">Er ging iets fout met het aanmaken van het gebruikersaccount probeer het later nog eens.</p>',
            };
    ...
    public function other_child_short_show(): bool|string
    {
        global $wp;

        $link = $_SESSION['redirect_url'] ?? home_url($wp);
        ob_start();
        ?>
     ...
     */
    public function account_short_show(): bool|string
    {
        global $wp;
    ...
    public function account_update_short_show(): bool|string
    {
        global $wp;
        $auth = service(Auth::class);
     ...
     * @throws NotFoundExceptionInterface
     */
    public function account_child_short_show(): void
    {
        global $wp;
        $auth = service(Auth::class);
        ...
        if (isset($_SESSION['error']['code'])) {
            ...
            echo match ($error['code']) {
                ...
                'GEBRUIKERSACCOUNT_BEZET' => '<p class="sb-text-center sb-login-warning">De opgegeven e-mailadres bestaat al.</p>',
                'ADRES_ONBEKEND'          => '<p class="sb-text-center sb-login-warning">Het opgegeven adress voor het opslaan van het gebruikersaccount is incorrect.</p>',
                'PERSOON_ID_ONGELDIG'     => '<p class="sb-text-center sb-login-warning">De persoon die u probeert bij te werken blijkt niet te bestaan probeer het later nog eens.</p>',
                default                   => '<p class="sb-text-center sb-login-warning">Er ging iets fout met het aanmaken van het gebruikersaccount probeer het later nog eens.</p>',
            };
        }
